...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden_list, h_activations, n_out):
        """Initialize the parameters for the multilayer perceptron
        ...
        h_size = len(n_hidden_list)
        self.hiddenLayer=[HiddenLayer]*h_size
        
        for ind in range(h_size):
            if ind == 0:
                self.hiddenLayer[ind] = HiddenLayer(
                    rng=rng,
                    input=input,
                    n_in=n_in,
                    n_out=n_hidden_list[ind],
                    activation=h_activations[ind]
                )
                continue
            ...
            if ind == h_size -1 :
                self.hiddenLayer[ind] = HiddenLayer(
                    rng=rng,
                    input=self.hiddenLayer[ind-1].output,
                    n_in=n_hidden_list[ind-1],
                    n_out=n_hidden_list[ind],
                    activation=h_activations[ind]
                )
                continue
        ...
        # of the hidden layer
        self.logRegressionLayer = LogisticRegression(
            input=self.hiddenLayer[h_size-1].output,
            n_in=n_hidden_list[h_size-1],
            n_out=n_out
        )
        ...
        l2_self_val = 0
        for ind in range(h_size):
            l1_self_val += abs(self.hiddenLayer[ind].W).sum()
            l2_self_val += (self.hiddenLayer[ind].W ** 2).sum()
        ...
        param_l = []
        for ind in range(h_size):
            param_l += self.hiddenLayer[ind].params
            
        self.params = param_l + self.logRegressionLayer.params
        # end-snippet-3
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=100,
             batch_size=20, n_hidden_list=[500], h_activations=[T.tanh], verbose=True ):
    """
    ...
    print ('hidden Layers = '+str(n_hidden_list))
