...
class RNNSLU2(object):
    """ Elman Neural Net Model Class
    """
    def __init__(self, nh1, nh2, nc, ne, de, cs, normal=True):
        """Initialize the parameters for the RNNSLU
        ...
        '''def recurrence(x_t, h_tm1, h_tm2):
            h1_t = T.nnet.sigmoid(T.dot(x_t, self.wx) + T.dot(h_tm1, self.wh) + self.bh)
            s1_t = T.nnet.softmax(T.dot(h1_t, self.w) + self.b)
            s1_t = s1_t.reshape((idxs.shape[0], nh1))
            h_t = T.nnet.sigmoid(T.dot(s1_t, self.wx2) + T.dot(h_tm2, self.wh2) + self.bh2)
            s_t = T.nnet.softmax(T.dot(h_t, self.w2) + self.b2)
            return [h1_t, s1_t, h_t, s_t]

        [h1, s1, h, s], _ = theano.scan(fn=recurrence,
                                sequences=x,
                                outputs_info=[self.h0, None, self.h1, None],
                                n_steps=x.shape[0])'''
        ...
        s1 = s1.reshape((idxs.shape[0], nh2))
        
        def recurrence2(s_t, h_tm2):
            h_t2 = T.nnet.sigmoid(T.dot(s_t, self.wx2) + T.dot(h_tm2, self.wh2) + self.bh2)
            s_t2 = T.nnet.softmax(T.dot(h_t2, self.w2) + self.b2)
            return [h_t2, s_t2]
        
        [h, s], _ = theano.scan(fn=recurrence2,
                                sequences=s1,
                                outputs_info=[self.h1, None],
                                n_steps=s1.shape[0])
...
class RNNSLU3(object):
    ...
    def __init__(self, nh, nc, ne, de, cs, normal=True):
        ...
        self.h0 = theano.shared(name='h0',
                                ...
                                dtype=theano.config.floatX))
        self.bh = theano.shared(name='h0',
                                value=numpy.zeros(nh,
                                dtype=theano.config.floatX))
        self.g = theano.shared(name='h0',
                                value=numpy.zeros(nh,
                                dtype=theano.config.floatX))
        # bundle
        self.params = [self.emb, self.wx, self.wh, self.w,
                       self.bh, self.g,  self.b, self.h0]
        ...
        def recurrence(x_t, h_tm1):
            ...
            ln_a_t = ln(a_t, self.bh, self.g)
            h_t = T.nnet.sigmoid(ln_a_t)
            s_t = T.nnet.softmax(T.dot(h_t, self.w) + self.b)
            return [h_t, s_t]
